# Spike A — Per-Profile Server-Side Enforcement

> Feasibility spike from the 2026-07-21 research fan-out. Investigates whether Jellyfin 10.10/10.11 offers ANY trustworthy server-side seam to enforce per-profile content filtering, and — if not — whether cleanyfin can "do it itself" safely without violating the metadata-only / super-easy / build-on-upstream constraints. Sourced from Jellyfin `master`/10.11.x source, official docs, and 10.11 breakage reports. Confidence tags: 🟢 verified from source/docs · 🟡 inferred, needs runtime test.
