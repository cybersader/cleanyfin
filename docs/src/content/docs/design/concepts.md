---
title: Concepts — The Primitives
description: A plain-language glossary of the load-bearing nouns the whole cleanyfin system is built from.
sidebar:
  order: 1
---

> Plain-language glossary of the nouns the whole system is built from. Backed by the deep-dives [taxonomy & data model](/cleanyfin/research/taxonomy/) and [federation architecture](/cleanyfin/research/federation/). For the concrete schema see [Data Model](/cleanyfin/design/data-model/); for the decision log see [Decisions](/cleanyfin/project/decisions/).
...
**How they fit:** a `SUBMITTER` tags a `SEGMENT` (with `CATEGORY`+`SEVERITY`+`ACTION`) on a `RELEASE` identified by a `FINGERPRINT`; a household's `PROFILE` and its subscribed `CURATOR`s decide which segments apply and what the player does, with `BYPASS` as the time-boxed override. The concrete tables and calibration tiers live in [Data Model](/cleanyfin/design/data-model/); how segments get created and moderated lives in [Contribution Workflows](/cleanyfin/design/contribution-workflows/); the honest tensions (skip-vs-mute, granularity, matching) in [Tradeoffs](/cleanyfin/project/tradeoffs/).
