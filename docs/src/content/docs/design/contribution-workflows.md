...
description: How a segment goes from "someone noticed something" to trusted, published, and mirrorable — marking, moderation, curators, and the automation gate.
sidebar:
  order: 4
---

> Backed by [federation architecture](/cleanyfin/research/federation/) and [taxonomy & data model](/cleanyfin/research/taxonomy/). Siblings: [Data Model](/cleanyfin/design/data-model/) (the schema being written), [Architecture](/cleanyfin/design/architecture/) (the three components), [Tradeoffs](/cleanyfin/project/tradeoffs/), [Concepts](/cleanyfin/design/concepts/).

The value of cleanyfin is the **shared, moderated dataset of tagged segments** — not any one client. This page covers how a segment goes from "someone noticed something" to "trusted, published, mirrorable." The whole pipeline is SponsorBlock's proven recipe (R08–R10), specialized to a family-safety tool where a *missed* filter breaks trust.
...
R01 (metadata-only), R04 (fingerprint keying), R05 (taxonomy), R08 (moderation), R09 (curators), R10 (automation gate). Open items that touch this flow: Sybil/rate-limit defense depth, whether mirrors ever accept upstream submissions — see [Open Questions](/cleanyfin/project/open-questions/).
