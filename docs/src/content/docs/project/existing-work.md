---
title: Existing Work & cleanyfin's Opening
description: The one direct competitor, the real standard behind it, and the four-way gap nobody has closed.
sidebar:
  order: 3
---

> 📎 Pointer stub. Backed by the deep-dive [prior-art-and-oss-competitors](/cleanyfin/research/prior-art-oss/). This is the "why does cleanyfin get to exist" file: the one direct competitor, the real standard behind it, and the four-way gap nobody has closed. For the full parts bin see [Prior Art](/cleanyfin/project/prior-art/).
...
Each capability exists in isolation (see [Prior Art](/cleanyfin/project/prior-art/)); the direct competitor has roughly one of them, weakly. cleanyfin = SponsorBlock's crowdsourcing/moderation/federation model + Jellyfin's native Media Segments enforcement + segment-editor's marking UX + MCF/EDL interop, on a boring one-`docker compose up` stack. (R02, R03, R06, R08, R09, R11)

## Honest caveats

- **Mute gap:** native clients skip only today; real mute needs the EDL bridge (Kodi/mpv) or upstream feature request [#3396](https://features.jellyfin.org/posts/3396/api-support-for-muting-media-for-content-filtering-plugin-support). (R07, [Trade-offs](/cleanyfin/project/tradeoffs/))
- **Enforcement unproven:** whether a 10.11 plugin enforces per-profile action server-side or is client-cooperative is Spike A — gates the architecture. ([Roadmap](/cleanyfin/project/roadmap/))
- **Federation is the hardest of the four** and easy to over-build — v1 is dumps + mirrors, not S2S protocols. (R03)

## Sources

Direct-competitor and MCF detail, plus the SponsorBlock/TheIntroDB/segment-editor analogs, are cited in the backing deep-dive: [prior-art-and-oss-competitors](/cleanyfin/research/prior-art-oss/) (findings F1, F2, F4, F5, F6; recommendation R6).
